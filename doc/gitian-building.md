...
*Setup instructions for a gitian build of ECRM using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ECRM
...
- [Building ECRM](#building-ecoreumcoin)
...
Clone the git repositories for ecoreumcoin and gitian and then checkout the ecoreumcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ecoreumcoin-crypto/ecoreumcoin.git
cd ecoreumcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ecoreumcoin repository
...
To build ECRM (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ecoreumcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ecoreumcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ecoreumcoin-crypto/ecoreumcoin
...
For example:
```bash
URL=https://github.com/crowning-/ecoreumcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ecoreumcoin=${COMMIT} --url ecoreumcoin=${URL} ../ecoreumcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ecoreumcoin=${COMMIT} --url ecoreumcoin=${URL} ../ecoreumcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ecoreumcoin=${COMMIT} --url ecoreumcoin=${URL} ../ecoreumcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ecoreumcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ecoreumcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ecoreumcoin-build.assert
```
...
[ecoreumcoin/gitian.sigs](https://github.com/ecoreumcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
