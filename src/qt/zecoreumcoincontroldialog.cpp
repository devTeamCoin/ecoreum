...
#include "ui_zecoreumcoincontroldialog.h"
...
std::list<std::string> ZEcoreumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEcoreumControlDialog::listMints;

ZEcoreumControlDialog::ZEcoreumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEcoreumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEcoreumControlDialog::~ZEcoreumControlDialog()
{
    delete ui;
}

void ZEcoreumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEcoreumControlDialog::updateList()
{
...
void ZEcoreumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEcoreumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEcoreum_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEcoreumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEcoreumControlDialog::GetSelectedMints()
{
...
void ZEcoreumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
