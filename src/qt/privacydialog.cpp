...
#include "zecoreumcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zECRM ought to be enough for anybody." - Bill Gates, 2017
    ui->zECRMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzECRMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzECRM->setEnabled(false);
        ui->pushButtonMintzECRM->setToolTip(tr("zECRM is currently disabled due to maintenance."));

        ui->pushButtonSpendzECRM->setEnabled(false);
        ui->pushButtonSpendzECRM->setToolTip(tr("zECRM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zECRMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzECRM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zECRM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zECRM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zECRM in ") + 
...
void PrivacyDialog::on_pushButtonSpendzECRM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zECRM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zECRM
        sendzECRM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zECRM
    sendzECRM();
}

void PrivacyDialog::on_pushButtonZEcoreumControl_clicked()
{
    ZEcoreumControlDialog* zEcoreumControl = new ZEcoreumControlDialog(this);
    zEcoreumControl->setModel(walletModel);
    zEcoreumControl->exec();
}

void PrivacyDialog::setZEcoreumControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEcoreumSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzECRM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ecoreum Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zECRMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zECRM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ECRM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zECRM</b>";
    ...
    // use mints from zEcoreum selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEcoreumControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEcoreumControlDialog::GetSelectedMints();
    }
    ...
    // Clear zecoreumcoin selector in case it was used
    ZEcoreumControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEcoreum Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ecoreum, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEcoreum Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zECRM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zECRM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zECRM "));
    ui->labelzECRMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzECRMSyncStatus->setVisible(fShow);
}
